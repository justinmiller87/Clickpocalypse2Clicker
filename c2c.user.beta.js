...
function loadSkipSettings() {
	...
	for (let charPos = 0; charPos < 5; charPos++) {
		skillPrioritySettings[charPos] = [0, 1, 2, 3].map(
			(col) => GM_getValue(`skillPriorityChar${charPos}Col${col}`, 0)
		);
	}
}
...
function appendSkillPriorityRow(container, charPos) {
	...
	for (let col = 0; col < 4; col++) {
		...
		colLabel.style.cssText = 'display: flex; align-items: center; margin-left: 10px; white-space: nowrap;';
...
function buildSettingsContent(container) {
	...
	appendSectionHeader(container, 'Character Skill Priority');

	const skillIntro = document.createElement('div');
	skillIntro.style.cssText = 'padding: 5px; margin-bottom: 8px; border: 1px solid #2B2B32; color: #AAA; font-size: 11px;';
	skillIntro.textContent = 'Each character\'s skill tree has 4 independent columns that each unlock top-to-bottom on their own pace, sharing one pool of skill points. Give a column priority 1, 2, 3... to have it claim the character\'s skill points before other columns. Leave a column at 0 to keep the default behavior (bought as part of the normal full-tree scan, in column order).';
	container.appendChild(skillIntro);

	for (let charPos = 0; charPos < 5; charPos++) {
		appendSkillPriorityRow(container, charPos);
	}
}
	...
	for (const charPos of pendingSkillCheckCharPositions) {
		...
		for (const priorityCol of getPrioritizedSkillColumns(charPos)) {
			for (let position = 0; position < 9; position++) {
				clickIt(`#characterSkillsContainer${charPos}_${position}_${priorityCol}_${position}`);
			}
		}
